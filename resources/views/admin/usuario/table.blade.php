<div style="background-color:var(--color6)" class="border rounded-xl px-4 shadow-2xl mt-8">
    <div class="flex mt-12 flex-row justify-end">
        <a href="{{ route('admin.user.new') }}">
            <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="currentColor" class="bi bi-person-add" viewBox="0 0 16 16">
                <path d="M12.5 16a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7m.5-5v1h1a.5.5 0 0 1 0 1h-1v1a.5.5 0 0 1-1 0v-1h-1a.5.5 0 0 1 0-1h1v-1a.5.5 0 0 1 1 0m-2-6a3 3 0 1 1-6 0 3 3 0 0 1 6 0M8 7a2 2 0 1 0 0-4 2 2 0 0 0 0 4"/>
                <path d="M8.256 14a4.5 4.5 0 0 1-.229-1.004H3c.001-.246.154-.986.832-1.664C4.484 10.68 5.711 10 8 10q.39 0 .74.025c.226-.341.496-.65.804-.918Q8.844 9.002 8 9c-5 0-6 3-6 4s1 1 1 1z"/>
            </svg>
        </a>
    </div>
    <div class="flex flex-row justify-center">
        <div class="xl:mt-16 mt-12" id="table">
            <table class="">
                <thead>
                    <th class=" xl:pr-12 hidden xl:inline pr-2">Nombre</th>
                    <th class=" xl:pr-12 hidden xl:inline pr-2">Apellidos</th>
                    <th class="xl:pr-12 pr-2">email</th>
                    <th class=" xl:pr-12 hidden xl:inline pr-2">teléfono</th>
                    <th class="xl:pr-12 pr-2">accion</th>
                </thead>
                <tbody id='tbodyUsuario'>
                    <tr>

                    </tr>
                </tbody>
            </table>
        </div>
    </div>

    <div class="flex flex-row justify-center mt-6">
        <div id="paginacionUser">
        </div>
    </div>
    <br><br>
</div>
